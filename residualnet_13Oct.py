...
class configuration(object):
    ...
    adv_n_layers = 2
    # dense_n_layers = 29
    # dense_hidden_layers = [30,30,30,30,30,30,30,30,
...
def read_parquet_data():
    ...
    train_features = train_data[training_feat1]
    ...
    test_features = test_data[training_feat1]
    ...
    val_features = val_data[training_feat1]
...
def residual_block(input_tensor, n_nodes, name, keep_prob, training, actvf=tf.math.tanh):
    bnargs = {'center':True, 'scale':True, 'training': training, 'fused': True, 'renorm':True}
    init = tf.contrib.layers.xavier_initializer()
    kernel_regularizer = tf.contrib.layers.l2_regularizer(configuration.weight_decay)

    with tf.variable_scope(name, initializer=init) as scope:
        
        x = input_tensor
        x = tf.layers.dense(x, units=n_nodes, activation=actvf, 
                            kernel_initializer=init, 
...
def residual_net(x, n_layers, hidden_layer_nodes, keep_prob, training, name, actvf, builder=residual_block, reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        #output = tf.layers.dropout(layers[-1], keep_prob, training=training)
        output = tf.layers.dense(layers[-1], units=1, kernel_initializer=init)
    
    return output

def adv_net_softmax_binning(output_dense_net, latent_vab1, 
                            latent_vab1_labels,
                            n_classes,
                            n_layers, 
                            hidden_nodes, 
...
def swish_sigmoid(x):
    ...
    activation_f = x*tf.math.sigmoid(x)
    return activation_f

def mean_squared_error(logits, labels):
    mse = tf.reduce_mean(tf.square(labels - logits))
...
class vanillaDNN(object):
    ...
    def __init__(self, config,
        ...
        self.adv_net_output = adv_net_softmax_binning(output_dense_net=tf.nn.softmax(self.dense_net_logits),
                                                      ...
                                                      actvf=tf.math.tanh,
                                                      training=self.training_phase)
        self.adv_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.deltaElabels, logits=self.adv_net_output))
        ...
        self.predictor_loss = mean_squared_error(logits=tf.math.sigmoid(self.dense_net_logits), 
                                                 labels=tf.expand_dims(tf.cast(self.label, tf.float32),1))
...
if __name__ == '__main__':
    ...
    x_train = pca.transform(featuresTrain)
